...
volatile uint8_t btnPressedFlag = 0; // 버튼 눌림 플래그 추가
...
int main(void)
{
  ...
  HAL_UART_Receive_IT(&huart6, &rx6char, 1); // 블루투스도 쓴다면 추가


  // 초기 상태 설정 (아래 2번 설명 참조)
  go_Straight();    // 일단 출발
  /* USER CODE END 2 */
  ...
  while (1)
  {
	  ...
	  if(btnPressedFlag == 1)
	  {
		// 1. 플래그 즉시 내림 (중복 실행 방지)
		btnPressedFlag = 0;
		HAL_GPIO_TogglePin(LD2_GPIO_Port, LD2_Pin);

		// 2. 데이터 전송 (이제 안전하게 보낼 수 있음)
		driveDirection = 0xFF;
		HAL_UART_Transmit(&huart6, (uint8_t*)&driveDirection, 1, 10); // Timeout을 100 정도로 넉넉히
	  ...
	  if(collisionFlag){
		 ...
		 {
			uart1StopFlag = 0xFF;
			// 1. 데이터가 바뀌기 전에 얼른 복사함 (Critical Section 대용)
			uint8_t currentDist = frontData;

			rxFrontDataFlag = 0; // 플래그 해제
...
{
	// [디버깅] 인터럽트가 걸리면 LED가 깜빡여야 함!
	// LD2_Pin이 없다면 연결하신 다른 LED 핀을 쓰세요.
	//HAL_GPIO_TogglePin(LD2_GPIO_Port, LD2_Pin);
...
void front_Event(uint8_t dist){
	...
	// 전역변수 frontData 대신, 넘겨받은 dist 사용
	if(dist < 30){
...
void HAL_UART_ErrorCallback(UART_HandleTypeDef *huart)
{
  ...
  // [추가] USART6 (충돌감지) 심폐소생술 - 이게 없어서 안 되는 것일 확률 매우 높음
  if(huart->Instance == USART6)
  {
      HAL_UART_Receive_IT(&huart6, &rx6char, 1);
  }
